...
def swipe_view(request):
    ...
    candidate = CustomUser.objects.exclude(id=me.id).exclude(id__in=already_swiped).exclude(is_blocked=True).select_related('profile').filter(profile__is_complete=True).first()
...
def matches_chat_view(request, user_id=None):
    ...
    raw_matches = Match.objects.filter(Q(user1=me)|Q(user2=me)).select_related('user1','user2')
    ...
    for m in raw_matches:
        ...
        unread = Message.objects.using('chat_db').filter(sender=other.id, receiver=me.id, is_read=False).count()
        match_list.append({
            'match': m,
            'other': other,
            'last_msg': last_msg,
    ...
    match_list.sort(key=lambda x: x['last_msg'].timestamp if x['last_msg'] else x['match'].created, reverse=True)
    ...
    if user_id:
        ...
        if active_match:
            if request.method == 'POST':
                ...
                if body:
                    Message.objects.using('chat_db').create(sender=me.id, receiver=active_user.id, body=body)
            ...
            Message.objects.using('chat_db').filter(sender=active_user.id, receiver=me.id, is_read=False).update(is_read=True)
...
def report_user(request, user_id):
    ...
    if request.method == 'POST':
        ...
        if reason in dict(Report.REASONS):
            ...
            messages.success(request, f'Report submitted. Our team will review it shortly.')
